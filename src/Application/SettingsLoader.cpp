	...
	for(unsigned int i = 0; i < numPresets; ++i){
		...
		for(int i = 0; i < numSurfaces; i++){
			if(xmlSettings->tagExists("surface", i)){
                ...
                if(xmlSettings->tagExists("control", 0)){                                       //AGREGADO MATIAS 07.2024
                    xmlSettings->pushTag("control");                                            //AGREGADO MATIAS 07.2024
                    std::string assignedKeyStr = xmlSettings->getValue("assignedKey", "");      //AGREGADO MATIAS 07.2024
                    sourceAssignedKey = assignedKeyStr.empty() ? '\0' : assignedKeyStr[0];      //AGREGADO MATIAS 07.2024
                    sourceOscAddress = xmlSettings->getValue("oscAddress", "/"+sourceName);                 //AGREGADO MATIAS 07.2024
                    if(xmlSettings->tagExists("speed", 0)){                                     //AGREGADO MATIAS 07.2024
                        speed = xmlSettings->getValue("speed", 24);                           //AGREGADO MATIAS 07.2024
                    }
                    xmlSettings->popTag(); // contrl                                            //AGREGADO MATIAS 07.2024
                }
...
bool SettingsLoader::save(SurfaceManager & surfaceManager, std::string fileName){
	...
	for(int i = 0; i < surfaces->size(); i++){
        ...
        xmlSettings->addTag("control");
        xmlSettings->pushTag("control");
        
        char assignedKey = surface->getAssignedKey();
        std::string keyStr(1, assignedKey);
        xmlSettings->addValue("assignedKey", keyStr);
        std::string oscAddress = surface->getOscAddress();
